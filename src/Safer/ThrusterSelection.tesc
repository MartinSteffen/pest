...
or ThrusterSelection:
	...
	or Selected_thrusters:
		...
		transitions:
			...
			from All_thrusters to BF_opt_delete on [!in(Global.IC_cmd_roll.0)];
			from All_thrusters to BF_final on [in(Global.IC_cmd_roll.0)];
			from BF_opt_delete to BF_final on BF_opt_delete_Ready;
			from BF_final to LRUD_final on [!in(Global.IC_cmd_pitch.0) && !in(Global.IC_cmd_yaw.0)];
			from BF_final to LRUD_opt_delete on [!(!in(Global.IC_cmd_pitch.0) && !in(Global.IC_cmd_yaw.0))];

...
//System.out.println("temprect4 : "+temprect4);
	...
	if (temprect1.intersects(temprect4))
			...
			trlist2.head = move_trans(trlist2.head, temprect3.x , temprect3.y );
			}  // hier noch arbeiten	
 	trlist = trlist.tail;
	}
...
public static void drawPar(Graphics g,int cx1, int cy1, int cx2, int cy2, Color c_color) 
 ...
 private Tr move_trans ( Tr trl, int dx, int dy ) {
  ...
  // System.out.println("verschiebe die Koordinaten um " + dx + " und " + dy);
      ...
      for (int i = 0; i < trl.points.length; i++) {

        trl.points[i].x = trl.points[i].x - dx;
        trl.points[i].y = trl.points[i].y - dy;

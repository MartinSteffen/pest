...
public class drawPESTConn 
{
	...
	if (matrix1.akt == matrix2.akt)
			...
			if (matrix1.akt == root.state) 
				...
				while (templist != null)
					...
					if (templist.head != matrix1.akt) 
					    {
						templist2 = templist3;
						templist3 = new StateList(templist.head,templist2);
					    }
				...
				if (matrix1.prev instanceof And_State ) 
				    {
					atemp2.substates = templist2;
				    } 
				if (matrix1.prev instanceof Or_State) 
				    { 
					otemp2.substates = templist2;
				    }
				drawConn(g,cx1,cy1,c_color);
			    }
		    }
	    ...
	    {
		//System.out.println("Conn Fehler");
